...
%                 pause
            ...
            generated_table((2-lane)*length(ds)*length(vi1s)*length(vi2s) + (vi2_i - 1)*length(vi1s)*length(ds) + (vi1_i - 1)*length(ds) + d_i,:) = [3-lane,d,vi1,vi2,col,round(x),round(vx),round(vehicles_pos(1,1) - w - d),t];
        end
    end
end
end
...
% Save the table generated
csvwrite(sprintf('data/gen_table_%d_%d_%d_%d_%d_%d.csv', ds(1), ds(length(ds)),vi1s(1), vi1s(length(vi1s)), vi2s(1), vi2s(length(vi2s))),generated_table)
