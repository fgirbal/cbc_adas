...
with open(sys.argv[2]) as csvfile:
	...
	for row in reader:
		if row["type"] == driver_type and row["lane"] == "1":
...
with open(sys.argv[2]) as csvfile:
	reader = csv.DictReader(csvfile)
	for row in reader:
		if row["type"] == driver_type and row["lane"] == "2":
...
f.write("	[] actrState = 1 & !crashed & !lC & lane = 1 & x <= length - v & t < max_time & positiveDist = false & (x1 + v1 - x - v) >= 6 -> 1:(x' = x + v) & (t' = t + 1) & (actrState' = 2);\n")
f.write("	[] actrState = 1 & !crashed & !lC & lane = 1 & x > length - v & t < max_time & positiveDist = false & (x1 + v1 - x - v) >= 6 -> 1:(x' = length) & (t' = t + 1) & (actrState' = 2);\n")
f.write("	[] actrState = 1 & !crashed & !lC & lane = 1 & x <= length - v & t < max_time & positiveDist = false & (x1 + v1 - x - v) < 6 -> 1:(x' = x + v) & (t' = t + 1) & (crashed' = true) & (actrState' = 2);\n")
f.write(" 	// The vehicle is in front of the other driver (positiveDist = true, x > x1)\n")
f.write("	[] actrState = 1 & !crashed & !lC & lane = 1 & x <= length - v & t < max_time & positiveDist = true -> 1:(x' = x + v) & (t' = t + 1) & (actrState' = 2);\n")
...
f.write("	[] actrState = 1 & !crashed & !lC & lane = 2 & x <= length - v & t < max_time -> 1:(x' = x + v) & (t' = t + 1) & (actrState' = 2);\n")
f.write("	[] actrState = 1 & !crashed & !lC & lane = 2 & x > length - v & t < max_time -> 1:(x' = length) & (t' = t + 1) & (actrState' = 2);\n\n")
...
with open(sys.argv[1]) as csvfile:
	...
	for row in reader:
		...
		if row["d"] != "40" and row["vi2"] == v1:
			...
			f.write(line)
			line = "	[] actrState = 1 & !crashed & lC & lane = %s & dist = %s & v = %s & x > length - %s & t <= max_time - %s -> 1:(crashed' = %s) & (x' = length) & (v' = %s) & (t' = t + %s) & (lane' = %s) & (actrState' = 2) & (lC' = false);\n" % (row["o_lane"], row["d"],row["vi1"],row["delta_x1"],row["delta_t"],acc_val,row["vf1"],row["delta_t"],str(3 - int(row["o_lane"])))
			f.write(line)
