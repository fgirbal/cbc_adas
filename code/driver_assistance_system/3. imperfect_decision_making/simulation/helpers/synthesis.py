...
def build_model(ex_path):
	...
	os.system('prism mdp_model.pm helpers/properties/verification_mod.pctl -exportmodel %s/out.all -exportresults %s/res.txt'%(ex_path, ex_path))
...
def synthesis(ex_path, query, output):
	...
	if query == "":
		...
		Tmin = int(f.readline())
		f.close()
		multi_obj_query = "multi(Pmax=? [F x=500 & t < %d], P<=0.3 [F crashed])"%Tmin
	else:
		multi_obj_query = query

	print('Synthesis using the query "%s"...'%multi_obj_query)

	os.system('prism -importmodel %s/out.all -lp -pctl "%s" -exportadv %s/%s.tra -exportprodstates %s/%s_new_states.sta -noprob1'%(ex_path,multi_obj_query,ex_path,output,ex_path,output))
