...
# in order to perform multi-objective synthesis for a
# scenario with two other vehicles.
#
# VERSION: 
...
# 17-Aug-2018; Last revision: 17-Aug-2018
...
parser.add_argument('v1', type=str, help='Initial velocity of the first other vehicle.')
parser.add_argument('x1_0', type=str, help='Initial position of the first other vehicle.')
parser.add_argument('v2', type=str, help='Initial velocity of the second other vehicle.')
parser.add_argument('x2_0', type=str, help='Initial position of the second other vehicle.')
...
if not (int(v) >= 15 and int(v) <= 34) or not (int(v1) >= 15 and int(v1) <= 34) or not (int(v2) >= 15 and int(v2) <= 34) or not (int(x1_0) >= 1 and int(x1_0) <= 500) or not (int(x2_0) >= 1 and int(x2_0) <= 500) or not (driver_type >= 1 and driver_type <= 3):
	raise ValueError("Input out of range.")

if int(x1_0) >= int(x2_0) or int(v1) > int(v2):
	raise ValueError("Not supported by this model; it must be that x1_0 < x2_0 and v1 <= v2.")
...
f.write("// Other vehicles\n")
f.write("const int v1 = %s; // do not alter this manually!\n"%v1)
f.write("const int x1_0 = %s;\n\n"%x1_0)
f.write("const int v2 = %s; // do not alter this manually!\n"%v2)
f.write("const int x2_0 = %s;\n\n"%x2_0)
...
f.write("global focusVehicle : [1..2] init 1;\n")
f.write("global lane : [1..2] init 1;\n\n")

f.write("formula x1 = x1_0 + v1*t;\n")
f.write("formula x2 = x2_0 + v2*t;\n")
# f.write("formula focusVehicle1 = (x <= x1);\n")
f.write("formula crashingCond = (focusVehicle = 1)?(x1 + v1 - x - v):(x2 + v2 - x - v);\n")
f.write("formula dist = (focusVehicle = 1)?(x > x1?(x - x1):(x1 - x)):(x > x2?(x - x2):(x2 - x));\n")
f.write("formula positiveDist = (focusVehicle = 1)?((x < length)?x > x1:true):((x < length)?x > x2:true);\n\n")
	...
	for row in reader:
		if row["d"] != max_dm_dist:
			for delta_a in a_vals:
				line = "	[] actrState = 1 & !crashed & !lC & lane = 1 & x <= length - v & t < max_time & positiveDist = false & crashingCond >= %s & v + a < 34 & v + a > 15 & dist = %s & v = %s & %s + %d <= 3 & %s + %d >= -3 -> 1:(x' = x + v) & (t' = t + 1) & (v' = v + a) & (a' = %s + %d) & (actrState' = 2);\n" % (crash_dist,row["d"],row["v"],row["a"],delta_a,row["a"],delta_a,row["a"],delta_a)
				f.write(line)
				line = "	[] actrState = 1 & !crashed & !lC & lane = 1 & x <= length - v & t < max_time & positiveDist = false & crashingCond >= %s & v + a >= 34 & dist = %s & v = %s & %s + %d <= 3 & %s + %d >= -3 -> 1:(x' = x + v) & (t' = t + 1) & (v' = 34) & (a' = %s + %d) & (actrState' = 2);\n" % (crash_dist,row["d"],row["v"],row["a"],delta_a,row["a"],delta_a,row["a"],delta_a)
				f.write(line)
				line = "	[] actrState = 1 & !crashed & !lC & lane = 1 & x <= length - v & t < max_time & positiveDist = false & crashingCond >= %s & v + a <= 15 & dist = %s & v = %s & %s + %d <= 3 & %s + %d >= -3 -> 1:(x' = x + v) & (t' = t + 1) & (v' = 15) & (a' = %s + %d) & (actrState' = 2);\n" % (crash_dist,row["d"],row["v"],row["a"],delta_a,row["a"],delta_a,row["a"],delta_a)
				f.write(line)
...
f.write("\n	[] actrState = 1 & !crashed & !lC & lane = 1 & x > length - v & t < max_time & positiveDist = false & crashingCond >= %s -> 1:(x' = length) & (t' = t + 1) & (actrState' = 2);\n"%crash_dist)
f.write("	[] actrState = 1 & !crashed & !lC & lane = 1 & x > length - v & t < max_time & positiveDist = false & crashingCond < %s -> 1:(x' = length) & (t' = t + 1) & (crashed' = true) & (actrState' = 2);\n\n"%crash_dist)

f.write("	[] actrState = 1 & !crashed & !lC & lane = 1 & x <= length - v & t < max_time & positiveDist = false & crashingCond < %s & v + a < 34 & v + a > 15 -> 1:(x' = x + v) & (t' = t + 1) & (v' = v + a) & (crashed' = true) & (actrState' = 2);\n"%crash_dist)
f.write("	[] actrState = 1 & !crashed & !lC & lane = 1 & x <= length - v & t < max_time & positiveDist = false & crashingCond < %s & v + a >= 34 -> 1:(x' = x + v) & (t' = t + 1) & (v' = 34) & (crashed' = true) & (actrState' = 2);\n"%crash_dist)
f.write("	[] actrState = 1 & !crashed & !lC & lane = 1 & x <= length - v & t < max_time & positiveDist = false & crashingCond < %s & v + a <= 15 -> 1:(x' = x + v) & (t' = t + 1) & (v' = 15) & (crashed' = true) & (actrState' = 2);\n"%crash_dist)	
...
with open(args.lane_change_table) as csvfile:
	...
	for row in reader:
		...
		if v1 == v2:
			...
			if row["vi2"] == v1:
				toWriteFocus = "& focusVehicle = 1 "
			else:
				toWriteFocus = "& focusVehicle = 2 "

		if row['o_lane'] == "2":
			changeFocus = " & (focusVehicle' = 2)"
		else:
			changeFocus = ""

		if probCrash != 0 and probCrash != 1:
			if row["d"] != max_control_dist and (row["vi2"] == v1 or row["vi2"] == v2):
				line = "	[] actrState = 1 & !crashed & lC %s& lane = %s & dist = %s & v = %s & x <= length - %s & t <= max_time - %s -> %.2f:(crashed' = true) & (actrState' = 2) & (lC' = false) & (k_chosen' = %d) + %.2f:(crashed' = false) & (x' = x + %s) & (v' = %s) & (t' = t + %s) & (a' = 0) & (lane' = %s) & (actrState' = 2) & (lC' = false) & (k_chosen' = %d)%s;\n" % (toWriteFocus, row["o_lane"], row["d"],row["vi1"],row["delta_x1"],row["delta_t"],probCrash,k,1-probCrash,row["delta_x1"],row["vf1"],row["delta_t"],str(3 - int(row["o_lane"])),k,changeFocus)
				f.write(line)
				line = "	[] actrState = 1 & !crashed & lC %s& lane = %s & dist = %s & v = %s & x > length - %s & t <= max_time - %s -> %.2f:(crashed' = true) & (actrState' = 2) & (lC' = false) & (k_chosen' = %d) + %.2f:(crashed' = false) & (x' = length) & (v' = %s) & (t' = t + %s) & (a' = 0) & (lane' = %s) & (actrState' = 2) & (lC' = false) & (k_chosen' = %d)%s;\n" % (toWriteFocus, row["o_lane"], row["d"],row["vi1"],row["delta_x1"],row["delta_t"],probCrash,k,1-probCrash,row["vf1"],row["delta_t"],str(3 - int(row["o_lane"])),k,changeFocus)
				f.write(line)
				line = "	[] actrState = 1 & !crashed & lC %s& lane = %s & dist = %s & v = %s & x <= length - %s & t > max_time - %s -> %.2f:(crashed' = true) & (actrState' = 2) & (lC' = false) & (k_chosen' = %d) + %.2f:(crashed' = false) & (x' = x + %s) & (v' = %s) & (t' = max_time) & (a' = 0) & (lane' = %s) & (actrState' = 2) & (lC' = false) & (k_chosen' = %d)%s;\n" % (toWriteFocus, row["o_lane"], row["d"],row["vi1"],row["delta_x1"],row["delta_t"],probCrash,k,1-probCrash,row["delta_x1"],row["vf1"],str(3 - int(row["o_lane"])),k,changeFocus)
				f.write(line)
				line = "	[] actrState = 1 & !crashed & lC %s& lane = %s & dist = %s & v = %s & x > length - %s & t > max_time - %s -> %.2f:(crashed' = true) & (actrState' = 2) & (lC' = false) & (k_chosen' = %d) + %.2f:(crashed' = false) & (x' = length) & (v' = %s) & (t' = max_time) & (a' = 0) & (lane' = %s) & (actrState' = 2) & (lC' = false) & (k_chosen' = %d)%s;\n" % (toWriteFocus, row["o_lane"], row["d"],row["vi1"],row["delta_x1"],row["delta_t"],probCrash,k,1-probCrash,row["vf1"],str(3 - int(row["o_lane"])),k,changeFocus)
				f.write(line)
			elif row["d"] == max_control_dist and (row["vi2"] == v1 or row["vi2"] == v2):
				line = "	[] actrState = 1 & !crashed & lC %s& lane = %s & dist >= %s & v = %s & x <= length - %s & t <= max_time - %s -> %.2f:(crashed' = true) & (actrState' = 2) & (lC' = false) & (k_chosen' = %d) + %.2f:(crashed' = false) & (x' = x + %s) & (v' = %s) & (t' = t + %s) & (a' = 0) & (lane' = %s) & (actrState' = 2) & (lC' = false) & (k_chosen' = %d)%s;\n" % (toWriteFocus, row["o_lane"], row["d"],row["vi1"],row["delta_x1"],row["delta_t"],probCrash,k,1-probCrash,row["delta_x1"],row["vf1"],row["delta_t"],str(3 - int(row["o_lane"])),k,changeFocus)
				f.write(line)
				line = "	[] actrState = 1 & !crashed & lC %s& lane = %s & dist >= %s & v = %s & x > length - %s & t <= max_time - %s -> %.2f:(crashed' = true) & (actrState' = 2) & (lC' = false) & (k_chosen' = %d) + %.2f:(crashed' = false) & (x' = length) & (v' = %s) & (t' = t + %s) & (a' = 0) & (lane' = %s) & (actrState' = 2) & (lC' = false) & (k_chosen' = %d)%s;\n" % (toWriteFocus, row["o_lane"], row["d"],row["vi1"],row["delta_x1"],row["delta_t"],probCrash,k,1-probCrash,row["vf1"],row["delta_t"],str(3 - int(row["o_lane"])),k,changeFocus)
				f.write(line)
			...
			if row["d"] != max_control_dist and (row["vi2"] == v1 or row["vi2"] == v2):
				line = "	[] actrState = 1 & !crashed & lC %s& lane = %s & dist = %s & v = %s & x <= length - %s & t <= max_time - %s -> 1:(crashed' = false) & (x' = x + %s) & (v' = %s) & (t' = t + %s) & (a' = 0) & (lane' = %s) & (actrState' = 2) & (lC' = false) & (k_chosen' = %d)%s;\n" % (toWriteFocus, row["o_lane"], row["d"],row["vi1"],row["delta_x1"],row["delta_t"],row["delta_x1"],row["vf1"],row["delta_t"],str(3 - int(row["o_lane"])),k,changeFocus)
				f.write(line)
				line = "	[] actrState = 1 & !crashed & lC %s& lane = %s & dist = %s & v = %s & x > length - %s & t <= max_time - %s -> 1:(crashed' = false) & (x' = length) & (v' = %s) & (t' = t + %s) & (a' = 0) & (lane' = %s) & (actrState' = 2) & (lC' = false) & (k_chosen' = %d)%s;\n" % (toWriteFocus, row["o_lane"], row["d"],row["vi1"],row["delta_x1"],row["delta_t"],row["vf1"],row["delta_t"],str(3 - int(row["o_lane"])),k,changeFocus)
				f.write(line)
				line = "	[] actrState = 1 & !crashed & lC %s& lane = %s & dist = %s & v = %s & x <= length - %s & t > max_time - %s -> 1:(crashed' = false) & (x' = x + %s) & (v' = %s) & (t' = max_time) & (a' = 0) & (lane' = %s) & (actrState' = 2) & (lC' = false) & (k_chosen' = %d)%s;\n" % (toWriteFocus, row["o_lane"], row["d"],row["vi1"],row["delta_x1"],row["delta_t"],row["delta_x1"],row["vf1"],str(3 - int(row["o_lane"])),k,changeFocus)
				f.write(line)
				line = "	[] actrState = 1 & !crashed & lC %s& lane = %s & dist = %s & v = %s & x > length - %s & t > max_time - %s -> 1:(crashed' = false) & (x' = length) & (v' = %s) & (t' = max_time) & (a' = 0) & (lane' = %s) & (actrState' = 2) & (lC' = false) & (k_chosen' = %d)%s;\n" % (toWriteFocus, row["o_lane"], row["d"],row["vi1"],row["delta_x1"],row["delta_t"],row["vf1"],str(3 - int(row["o_lane"])),k,changeFocus)
				f.write(line)
			elif row["d"] == max_control_dist and (row["vi2"] == v1 or row["vi2"] == v2):
				line = "	[] actrState = 1 & !crashed & lC %s& lane = %s & dist >= %s & v = %s & x <= length - %s & t <= max_time - %s -> 1:(crashed' = false) & (x' = x + %s) & (v' = %s) & (t' = t + %s) & (a' = 0) & (lane' = %s) & (actrState' = 2) & (lC' = false) & (k_chosen' = %d)%s;\n" % (toWriteFocus, row["o_lane"], row["d"],row["vi1"],row["delta_x1"],row["delta_t"],row["delta_x1"],row["vf1"],row["delta_t"],str(3 - int(row["o_lane"])),k,changeFocus)
				f.write(line)
			...
			if row["d"] != max_control_dist and (row["vi2"] == v1 or row["vi2"] == v2):
				line = "	[] actrState = 1 & !crashed & lC %s& lane = %s & dist = %s & v = %s & x <= length - %s & t <= max_time - %s -> 1:(crashed' = true) & (actrState' = 2) & (lC' = false) & (k_chosen' = %d);\n" % (toWriteFocus, row["o_lane"], row["d"],row["vi1"],row["delta_x1"],row["delta_t"],k)
				f.write(line)
				line = "	[] actrState = 1 & !crashed & lC %s& lane = %s & dist = %s & v = %s & x > length - %s & t <= max_time - %s -> 1:(crashed' = true) & (actrState' = 2) & (lC' = false) & (k_chosen' = %d);\n" % (toWriteFocus, row["o_lane"], row["d"],row["vi1"],row["delta_x1"],row["delta_t"],k)
				f.write(line)
				line = "	[] actrState = 1 & !crashed & lC %s& lane = %s & dist = %s & v = %s & x <= length - %s & t > max_time - %s -> 1:(crashed' = true) & (actrState' = 2) & (lC' = false) & (k_chosen' = %d);\n" % (toWriteFocus, row["o_lane"], row["d"],row["vi1"],row["delta_x1"],row["delta_t"],k)
