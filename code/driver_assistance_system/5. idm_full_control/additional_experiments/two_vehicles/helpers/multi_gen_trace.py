...
parser.add_argument('v1', type=int, help='Initial speed of the first vehicle.')
parser.add_argument('x1_0', type=int, help='Initial position of the first vehicle.')
parser.add_argument('v2', type=int, help='Initial speed of the second vehicle.')
parser.add_argument('x2_0', type=int, help='Initial position of the second vehicle.')
...
def multi_generate_trace_from_file(file, v1, x1_0, v2, x2_0, out):
	...
	with open(file) as csvfile:
		...
		for row in reader:
			...
			if row['actrState'] == "2" and next_change_lanes == True:
				...
				if row['focusVehicle'] == "1":
					idx_line = (2-lane)*20*20*43*3 + (v1 - 15)*20*43*3 + (curr_v - 15)*43*3 + (min(abs(x1_0 + v1*curr_t - curr_x), 43) - 1)*3 + int(row['k_chosen'])
				else:
					idx_line = (2-lane)*20*20*43*3 + (v2 - 15)*20*43*3 + (curr_v - 15)*43*3 + (min(abs(x2_0 + v2*curr_t - curr_x), 43) - 1)*3 + int(row['k_chosen'])
...
multi_generate_trace_from_file("%s/trace.txt"%args.path, args.v1, args.x1_0, args.v2, args.x2_0, "%s/%s"%(args.path,args.output))
if args.clear:
