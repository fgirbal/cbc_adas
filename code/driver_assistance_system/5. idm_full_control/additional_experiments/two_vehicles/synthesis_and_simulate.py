...
parser.add_argument('v', type=int, default=24, help='Initial velocity of the vehicle.')
parser.add_argument('v1', type=int, default=18, help='Initial velocity of the other vehicle.')
parser.add_argument('x1_0', type=int, default=30, help='Initial position of the other vehicle.')
parser.add_argument('v2', type=int, default=20, help='Initial velocity of the other vehicle.')
parser.add_argument('x2_0', type=int, default=150, help='Initial position of the other vehicle.')
...
if args.path:
	...
	p = "results/r_%d_%d_%d_%d_%d_%d"%(driver_type,v,v1,x1_0,v2,x2_0)
...
if not args.read_trace:
	...
	if not args.read_adv:
		# Synthesis
		if not args.query:
			os.system("python3 helpers/synthesis.py %d %d %d %d %d %d -p %s -o %s"%(driver_type, v, v1, x1_0, v2, x2_0, p, output))
		else:
			os.system('python3 helpers/synthesis.py %d %d %d %d %d %d -p %s -q "%s" -o %s'%(driver_type, v, v1, x1_0, v2, x2_0, p, args.query, output))
	else:
	...
	os.system("python3 helpers/multi_gen_trace.py %s %s %s %s %d %d %d %d -p %s"%(states_file, labels_file, new_states_file, adv_file, v1, x1_0, v2, x2_0, p))

else:
	print("Reading existing trace...")

os.system("python3 helpers/simulate.py %d %d %d %d %d %s/gen_trace.csv -x %f"%(v, v1, x1_0, v2, x2_0, p, speed))
