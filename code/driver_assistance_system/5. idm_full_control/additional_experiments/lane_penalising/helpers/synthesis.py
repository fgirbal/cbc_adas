...
def build_model(ex_path):
	...
	os.system('prism mdp_model.pm helpers/properties/lane_penalising.pctl -exportmodel %s/out.all -exportresults %s/res.txt -javamaxmem 4g -cuddmaxmem 2g'%(ex_path, ex_path))
...
def synthesis(ex_path, query, output):
	...
	if query == "":
		...
		multi_obj_query = "multi(Pmin=? [F crashed], R<=%.2f [C])"%min_prob
	else:
