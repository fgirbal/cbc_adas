...
In relation to the probabilistic operator in PCTL, the generalisation from the Markov chain to the Markov decision process can be done using the concept of adversary (also denoted strategy or scheduler), initially introduced in Section~\ref{sec:dtmcs_mdps}.

It should be mentioned that, in the case of an MDP, the operator $\mathbb{P}_{\triangleright p}$ should be interpreted under the following semantics:
...
In order to reason over bounds of conditional probabilities in limited scenarios, the following proposition is stated and its proof is presented.

\begin{proposition}
Consider a Markov decision process $\mathcal{M} = (S, Act, \mathbf{P}, p_{init}, AP, L)$ which admits adversaries $\sigma \in Adv$. Let $\varphi_1, \varphi_2$ be two PCTL path formulas, such that, for any path $\pi \in Paths(s)$: $\pi \models \varphi_1 \Rightarrow \pi \models \varphi_2$. In this case it holds that:
...
Assume that $\sigma' \in Adv$ is an adversary such that:

\begin{equation}
	\sigma' \in \arg\sup_{\sigma \in Adv} P^{\sigma}_s (\varphi_1 \bigmid \varphi_2)
\end{equation}

that is, $\sigma'$ is an adversary under which the probability of $\varphi_1 \bigmid \varphi_2$ is maximised. Under this adversary, it is true, through Bayes' theorem of conditional probability:
...
Consider $\sigma_1, \sigma_2 \in Adv$ such that:

\begin{equation}
\label{eq:max_sigmas}
\begin{aligned}
	& \sigma_1 \in \arg\sup_{\sigma \in Adv} P^{\sigma}_s (\varphi_1) \\
	& \sigma_2 \in \arg\sup_{\sigma \in Adv} P^{\sigma}_s (\varphi_2)
\end{aligned}
\end{equation}

Assume, for the safe of contradiction, that:
...
For this to be true, it must be that $P^{\sigma'}_s (\varphi_1) < p_{\max, s} (\varphi_1)$ and/or $P^{\sigma'}_s (\varphi_2) > p_{\max, s} (\varphi_2)$. The second condition constitutes a contradiction, since in that case $\sigma_2$ would not maximise the probability of $\varphi_2$ ($p_{\max, s} (\varphi_2) = P^{\sigma_2}_s (\varphi_2)$, by definition), a violation of the assumption described in Equation~\ref{eq:max_sigmas}. We are thus left with the conditions $P^{\sigma'}_s (\varphi_1) < p_{\max, s} (\varphi_1) \wedge P^{\sigma'}_s (\varphi_2) \leq p_{\max, s} (\varphi_2)$. % However, this also poses a contradiction on the assumption that $\sigma'$ maximises the probability of $\varphi_1 \bigmid \varphi_2$, as in that case the strategy $\sigma_1$ yields a higher probability, given that $(\pi \models \varphi_1 \Rightarrow \pi \models \varphi_2) \Rightarrow \forall \sigma \in Adv: P^{\sigma}_s (\varphi_2) \geq P^{\sigma}_s (\varphi_1)$.

%And thus we can state that:

%\begin{equation}
%	\frac{P^{\sigma'}_s (\varphi_1)}{P^{\sigma'}_s (\varphi_2)} < \frac{P^{\sigma'_1}_s (\varphi_1)}{P^{\sigma'_1}_s (\varphi_2)}
%\end{equation}

%For this to be true, it must stand that $P^{\sigma'}_s (\varphi_1) < P^{\sigma'_1}_s (\varphi_1)$ and/or $P^{\sigma'}_s (\varphi_2) > P^{\sigma'_1}_s (\varphi_2)$. This means that, under $\sigma'_1$, either $\varphi_1$ or $\varphi_2$ have a higher probability of being satisfied than under $\varphi$. 

%Since it must hold that $\pi \in Paths(s)$: $\pi \models \varphi_1 \Rightarrow \pi \models \varphi_2$, it should also be that $P^{\sigma'}_s (\varphi_2) \geq P^{\sigma'}_s (\varphi_1)$ and $P^{\sigma'_1}_s (\varphi_2) \geq P^{\sigma'_1}_s (\varphi_1)$. Therefore, if $P^{\sigma'}_s (\varphi_1) < P^{\sigma'_1}_s (\varphi_1)$, then:

%\begin{equation}
%	P^{\sigma'}_s (\varphi_1) < P^{\sigma'_1}_s (\varphi_1) \leq P^{\sigma'_1}_s (\varphi_2) < P^{\sigma'}_s (\varphi_2)
%\end{equation}
